...
The recipient never sees raw ciphertext or tags — just the link, then the plaintext. The secret travels as opaque base64url in the query string; the password never does. Only ciphertext is exposed in the URL — without the level password it cannot be decrypted.
